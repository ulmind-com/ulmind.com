...
const CRMInvoicesPage = lazy(() => import("./admin/pages/crm/InvoicesPage"));
const CRMPaymentsPage = lazy(() => import("./admin/pages/crm/PaymentsPage"));
...
const WhatsAppFloat: React.FC = () => {
  ...
  const [waAnimData, setWaAnimData] = useState<object | null>(null);
  const [callAnimData, setCallAnimData] = useState<object | null>(null);
  const location = useLocation();

  useEffect(() => {
    // Fetch WhatsApp animation
    fetch('/lottieflow-social-networks-15-11-25D366-easey.json')
      .then((r) => r.json())
      .then((d: any) => {
        const stripped = {
          ...d,
        ...
        setWaAnimData(stripped);
      })
      .catch(() => {});

    // Fetch Call animation
    fetch('/lottieflow-ecommerce-14-18-0567eb-easey.json')
      .then((r) => r.json())
      .then((d: any) => {
        const stripped = {
          ...d,
  ...
  return (
    <>
      ...
      >
        {callAnimData ? (
          <Lottie
            animationData={callAnimData}
            loop
            autoplay
            style={{ width: 56, height: 56 }}
          />
        ) : (
          <Phone size={26} color="#3B82F6" />
        )}
      </a>
